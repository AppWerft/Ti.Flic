 ...
 * Flic Manager.
 *
 * This is a singleton class used to communicate with the Flic Application. Use
 * {@link FlicManager#getInstance(Context, FlicManagerInitializedCallback, FlicManagerUninitializedCallback)}
 * or {@link FlicManager#getInstance(Context, FlicManagerInitializedCallback)}
 * to retrieve a manager.
 */
public final class FlicManager {
	...
	private List<FlicManagerInitializedCallback> mInitializedCallbacks = new ArrayList<FlicManagerInitializedCallback>();
	private List<FlicManagerUninitializedCallback> mUninitializedCallbacks = new ArrayList<FlicManagerUninitializedCallback>();
	private final HashMap<String, FlicButton> mKnownButtons = new HashMap<String, FlicButton>();
	 ...
	 *
	 * The credentials can only be set once. Setting them again is a no-op.
	 *
	 * @param appId
	 *            App ID
	 * @param appSecret
	 *            App Secret
	 * @param appName
	 *            App name that will be shown to the user in the Flic App
	 */
	...
	static boolean hasSetAppCredentials() {
		return instance.mAppId != null && instance.mAppSecret != null
				&& instance.mAppName != null;
	}
	 ...
	 *
	 * @param context
	 *            An application/service/activity context.
	 *            {@link Context#getApplicationContext()} on this object will be
	 *            the context to use.
	 * @param initializedCallback
	 *            A callback that will be called with the manager as parameter.
	 *
	 * @throws IllegalArgumentException
	 *             If context or initializedCallback is null.
	 * @throws AppCredentialsNotProvidedException
	 *             If App credentials were not provided. See
	 *             {@link #setAppCredentials(String, String, String)}.
	 * @throws FlicAppNotInstalledException
	 *             If the Flic Application is not installed.
	 */
	 ...
	 * @param initializedCallback
	 *            A callback that will be called with the manager as parameter.
	 * @param uninitializedCallback
	 *            If non-null, a callback that will be called if the Flic
	 *            Application exits, or when
	 *            {@link FlicManager#destroyInstance()} is called.
	 *
	 * @throws IllegalArgumentException
	 *             If context or initializedCallback is null.
	 * @throws AppCredentialsNotProvidedException
	...
	public static void getInstance(Context context,
		...
		instance.getInstanceInternal(context, initializedCallback,
				uninitializedCallback);
	}

	private void getInstanceInternal(Context context,
			FlicManagerInitializedCallback initializedCallback,
			FlicManagerUninitializedCallback uninitializedCallback) {
		if (!hasSetAppCredentials()) {
			throw new AppCredentialsNotProvidedException(
					"App credentials were not provided");
		}
	...
	IFlicLibCallbackInterface mCallbackIntf = new IFlicLibCallbackInterface.Stub() {
		...
		public void onConnect(String mac) {
			...
			if (button != null) {
				synchronized (button.callbacks) {
					for (FlicButtonCallback cb : (ArrayList<FlicButtonCallback>) button.callbacks
							.clone()) {
		...
		@Override
		public void onButtonUpOrDown(String mac, boolean wasQueued,
				int timeDiff, int action) {
			...
			if (button != null) {
				synchronized (button.callbacks) {
					for (FlicButtonCallback cb : (ArrayList<FlicButtonCallback>) button.callbacks
							.clone()) {
						cb.onButtonUpOrDown(button, wasQueued, timeDiff,
								action == 0, action == 1);
					}
				}
			}
		...
		@Override
		public void onButtonClickOrHold(String mac, boolean wasQueued,
				int timeDiff, int action) {
			mac = mac.toLowerCase();
			FlicButton button = synchronizedGetButton(mac);
			if (button != null) {
				synchronized (button.callbacks) {
					for (FlicButtonCallback cb : (ArrayList<FlicButtonCallback>) button.callbacks
							.clone()) {
						cb.onButtonClickOrHold(button, wasQueued, timeDiff,
								action == 0, action == 1);
					}
				}
			}
		...
		@Override
		public void onButtonSingleOrDoubleClick(String mac, boolean wasQueued,
				int timeDiff, int action) {
			mac = mac.toLowerCase();
			FlicButton button = synchronizedGetButton(mac);
			if (button != null) {
				synchronized (button.callbacks) {
					for (FlicButtonCallback cb : (ArrayList<FlicButtonCallback>) button.callbacks
							.clone()) {
						cb.onButtonSingleOrDoubleClick(button, wasQueued,
								timeDiff, action == 0, action == 1);
					}
				}
			}
		}
		...
		@Override
		public void onButtonSingleOrDoubleClickOrHold(String mac,
				boolean wasQueued, int timeDiff, int action) {
			mac = mac.toLowerCase();
			FlicButton button = synchronizedGetButton(mac);
			if (button != null) {
				synchronized (button.callbacks) {
					for (FlicButtonCallback cb : (ArrayList<FlicButtonCallback>) button.callbacks
							.clone()) {
						cb.onButtonSingleOrDoubleClickOrHold(button, wasQueued,
								timeDiff, action == 0, action == 1, action == 2);
					}
				}
			}
		}
	...
	private void init(Context context, FlicManagerCallback flicManagerCallback) {
		...
		mServiceConnection = new ServiceConnection() {
			...
			public void onServiceConnected(ComponentName name, IBinder service) {
				synchronized (mIntfLock) {
					IFlicLibInterface intf = IFlicLibInterface.Stub
							.asInterface(service);
					try {
						mIntfId = intf.registerCallback(mCallbackIntf, mAppId,
								mAppSecret, mAppName);
						mIntf = intf;
					...
					try {
						...
						for (String address : addresses) {
							if (verifiedButtons.contains(address)
									&& !mKnownButtons.containsKey(address)) {
								mKnownButtons.put(address, new FlicButton(
										FlicManager.this, address));
							}
						}
						for (FlicButton button : mKnownButtons.values()) {
							mIntf.listenForConnectionCallbacks(mIntfId,
									button.mac);
							mIntf.setButtonCallbacks(mIntfId, button.mac,
									button.callbackFlags);
						}
					} catch (RemoteException e) {
						e.printStackTrace();
					} catch (RuntimeException e) {
		...
		if (!context.bindService(intent, mServiceConnection,
				Context.BIND_AUTO_CREATE)) {
			isInitializing = false;
			mInitializedCallbacks.clear();
			mUninitializedCallbacks.clear();
			throw new FlicAppNotInstalledException(
					"Flic Application is not installed");
		}
	}

	/**
	 * Clean resources and unregister all callbacks. Use
	 * {@link FlicManager#getInstance} to re-initialize.
	 */
	 ...
	 *
	 * @param deviceId
	 *            The Bluetooth device address case insensitive.
	 ...
	 *
	 * This is a list of all previously grabbed buttons that have not been
	 * removed.
	 *
	 * @return The list
	 */
	public List<FlicButton> getKnownButtons() {
		synchronized (mKnownButtons) {
			return new ArrayList<FlicButton>(mKnownButtons.values());
		}
	}
	 ...
	 *
	 * Call this method to ask the Flic app to retrieve a button object. This
	 * calls startActivityForResult with the request code
	 * {@link FlicManager#GRAB_BUTTON_REQUEST_CODE}. The result retrieved in
	 * onActivityResult for currentActivity must be sent to the
	 * completeGrabButton method.
	 *
	...
	/**
	 * Completes a grab button sequence. Call this from
	 * {@link Activity#onActivityResult(int, int, Intent)}.
	 *
	 * @param data
	 *            An intent received by onActivityResult
	 * @return The grabbed FlicButton, null on failure, if requestCode wasn't
	 *         {@link FlicManager#GRAB_BUTTON_REQUEST_CODE} or if resultCode was
	 *         not {@link Activity#RESULT_OK}.
	 */
	public FlicButton completeGrabButton(int requestCode, int resultCode,
			Intent data) {
		if (requestCode != GRAB_BUTTON_REQUEST_CODE
		...
		if (buttonPublicCurve25519Key == null
				|| buttonPublicCurve25519Key.length != 32) {
			Log.e(TAG, "completeGrabButton: Invalid pubkey "
					+ (buttonPublicCurve25519Key == null ? "null" : "length "
							+ buttonPublicCurve25519Key.length));
			return null;
		}
					...
					.generatePublic(new RSAPublicKeySpec(
							new BigInteger(
									""
											...
											+ "8883202203423153080919807757020166001248942233209849142898672366749341514289888739"),
		...
		Curve25519.curve(sharedSecret, mLastPrivateCurve25519Key,
				buttonPublicCurve25519Key);
			...
			synchronized (mIntfLock) {
				if (mIntf != null) {
					try {
						...
						mIntf.setButtonCallbacks(mIntfId, mac,
								flicButton.callbackFlags);
					} catch (RemoteException e) {
						e.printStackTrace();
					}
				}
	...
	/**
	 * Forget button. Disposes a button object and removes it from the internal
	 * list of known buttons. You will no longer get callbacks for this button.
	 *
	 * @param button
	 *            A button
	 */
				...
				synchronized (mIntfLock) {
					if (mIntf != null) {
						try {
							...
							mIntf.unListenForConnectionCallbacks(mIntfId,
									button.mac);
						} catch (RemoteException e) {
							e.printStackTrace();
						}
